...
const maskingConfigSchema = zod.z
    ...
    fields: zod.z.array(zod.z.union([zod.z.string(), zod.z.instanceof(RegExp)])).optional(),
    ...
    maxDepth: zod.z.number().int().positive().optional(),
    /**
     * The masking style.
     ...
     */
    style: zod.z.enum(['fixed', 'preserve-length']).optional(),
})
    .optional();
/**
...
const syntropyLogConfigSchema = zod.z.object({
    ...
    doctor: zod.z
        ...
        disableRules: zod.z.array(zod.z.string()).optional(),
    })
        .optional(),
});
            ...
            this.storage.run({ data: newContextData }, async () => {
                try {
                    ...
                    if (!this.get('correlationId')) {
                        this.set('correlationId', crypto.randomUUID());
                    }
...
class Logger {
    ...
    async _log(level, ...args) {
        ...
        if (formatString) {
            logEntry.message = util__namespace.format(formatString, ...formatArgs);
        }
        ...
        switch (config.mode) {
            ...
            case 'sentinel': {
                ...
                if (config.mode === 'single') {
                    ...
                    return redis.createClient(sentinelOptions);
                }
            }
            case 'cluster': {
                ...
                return redis.createClient(clusterOptions);
            }
            default: {
 ...
 */
class SyntropyLog extends events.EventEmitter {
    static instance;
    lifecycleManager;
    constructor() {
        super();
